package io.github.cheezeplaylist.controller;

import io.github.cheezeplaylist.dto.*;
import io.github.cheezeplaylist.service.impl.MusicServiceImpl;
...
public class APIController {
    ...
    public ResponseEntity<?> identicalUsername(@RequestParam String username) {
        ...
        if (identicalUsername == null) return ResponseEntity.ok().body("undefined");
    ...
    public ResponseEntity<?> findUsername(@RequestParam String username) {
        ...
        if (findPassword == null) return ResponseEntity.status(201).body("회원가입이 되어있지 않습니다.");
    ...
    public ResponseEntity<?> findEmail(@RequestParam String email) {
        ...
        if (findUserId == null) return ResponseEntity.status(201).body("회원가입이 되어있지 않습니다.");
    ...
    public ResponseEntity<?> findChannel(@RequestParam String chzzkChannelId, SecurityContext context) {
        ...
        if (!channelId.equals("(알 수 없음)")) {
            for (PlaylistDto dto : dtoList) {
                if (Objects.equals(dto.getChzzkChannelId(), chzzkChannelId)) return ResponseEntity.ok().body(dto.getChzzkChannelId());
            }
        }
    ...
    public ResponseEntity<?> message(@DestinationVariable("playlistId") String playlistId, MessageRequestDto messageRequestDTO) {
        log.info("음악: {}", messageRequestDTO.getTitle());
